...
#include <stdint.h>
#include <vector>
#include "pin/pin.H"  // For INS type
 ...
 */
enum UopType : uint8_t {UOP_GENERAL, UOP_LOAD, UOP_STORE, UOP_STORE_ADDR, UOP_MAGIC_OP, UOP_DEQUEUE, UOP_FENCE};
...
struct BblInfo {
    uint32_t instrs;
    uint64_t addr;
    // 0 bytes, but will be 1-sized when we have an element
...
class Decoder {
    ...
    public:
        ...
        static BblInfo* decodeBbl(BBL bbl, INS start, INS end);
    ...
    private:
        ...
        static void emitLoad(const Instr&, uint32_t idx,
                             DynUopVec& uops, uint32_t destReg = 0);
        static void emitStore(const Instr&, uint32_t idx,
                              DynUopVec& uops, uint32_t srcReg = 0);

        //Emit all loads and stores for this uop
        static void emitLoads(const Instr&, DynUopVec& uops);
        static void emitStores(const Instr&, DynUopVec& uops);
        ...
        static void emitBasicMove(const Instr&, DynUopVec& uops,
                                  uint32_t lat, uint8_t ports);
        static void emitConditionalMove(const Instr&, DynUopVec& uops,
                                        uint32_t lat, uint8_t ports);

        // 1 "exec" uop, 0-2 inputs, 0-2 outputs
        static void emitBasicOp(const Instr&, DynUopVec& uops,
                                uint32_t lat, uint8_t ports,
                                uint8_t extraSlots = 0,
                                bool reportUnhandled = true);
        ...
        static void emitChainedOp(const Instr&, DynUopVec& uops,
                                  uint32_t numUops,
                                  uint32_t* latArray, uint8_t* portsArray);

        // Some convert ops need 2 chained exec uops, though they have a single input and output
        static void emitConvert2Op(const Instr&, DynUopVec& uops,
                                   uint32_t lat1, uint32_t lat2,
                                   uint8_t ports1, uint8_t ports2);

        /* Specific cases */
        static void emitXchg(const Instr&, DynUopVec& uops);
        static void emitMul(const Instr&, DynUopVec& uops);
        static void emitDiv(const Instr&, DynUopVec& uops);

        static void emitCompareAndExchange(const Instr&, DynUopVec&);

        /* Other helper functions */
        static void reportUnhandledCase(const Instr&, const char* desc);
        static void populateRegArrays(const Instr&,
                                      uint32_t* srcRegs,
                                      uint32_t* dstRegs);
