...
template<typename T> static void writeVar(libconfig::Setting& setting, const char* key, T val) {
    ...
    if (sep) {
        ...
        if (!setting.exists(prefix)) {
            ...
            } catch (const libconfig::SettingNameException& sne) {
        ...
        if (!setting.exists(key)) {
            ...
            //If this panics, what are you doing in the code? Multiple reads and different defaults??
...
template<typename T>
T Config::genericGet(const char* key, T def) const {
    T val;
    if (inCfg->exists(key)) {
...
template<typename T>
T Config::genericGet(const char* key) const {
    T val;
    if (inCfg->exists(key)) {
        if (!inCfg->lookupValue(key, val)) {
...
template<> uint32_t Config::get<uint32_t>(const char* key) const {return (uint32_t) genericGet<int>(key);}
template<> uint64_t Config::get<uint64_t>(const char* key) const {return (uint64_t) genericGet<lc_int64>(key);}
template<> bool Config::get<bool>(const char* key) const {return genericGet<bool>(key);}
template<> const char* Config::get<const char*>(const char* key) const {return genericGet<const char*>(key);}
template<> double Config::get<double>(const char* key) const {return (double) genericGet<double>(key);}

template<> uint32_t Config::get<uint32_t>(const char* key, uint32_t def) const {return (uint32_t) genericGet<int>(key, (int)def);}
template<> uint64_t Config::get<uint64_t>(const char* key, uint64_t def) const {return (uint64_t) genericGet<lc_int64>(key, (lc_int64)def);}
template<> bool Config::get<bool>(const char* key, bool def) const {return genericGet<bool>(key, def);}
template<> const char* Config::get<const char*>(const char* key, const char* def) const {return genericGet<const char*>(key, def);}
template<> double Config::get<double>(const char* key, double def) const {return (double) genericGet<double>(key, (double)def);}

//Get subgroups in a specific key
void Config::subgroups(const char* key, std::vector<const char*>& grps) const {
    if (inCfg->exists(key)) {
