...
class HDF5BackendImpl {
    private:
        ...
        void dumpWalk(Stat* s) {
            ...
            if (AggregateStat* as = dynamic_cast<AggregateStat*>(s)) {
                ...
                *(curPtr++) = ss->get();
                ...
                auto moments = rs->getMoments();
                *(curPtr++) = std::get<0>(moments);
                *(curPtr++) = std::get<1>(moments);
                *(curPtr++) = std::get<2>(moments);
            } else {
        ...
        hid_t getBaseH5Type(Stat* s) {
            ...
            if (VectorStat* vs = dynamic_cast<VectorStat*>(s)) {
                size = vs->size();
            } else if (dynamic_cast<RunningStat<>*>(s)) {
                size = 3;  // 3 moments
            }
    ...
    public:
        ...
        {
            // Create stats file
            DEBUG("HDF5 backend: Opening %s", filename);
            ...
            dataBuf = static_cast<uint64_t*>(malloc(bufSize));
            curPtr = dataBuf;

            bufferedRecords = 0;

            DEBUG("HDF5 backend: Created table, %ld bytes/record, %d records/write", recordSize, recordsPerWrite);
            H5Fclose(fileID);
        }
